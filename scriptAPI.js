    ...
    function getCoordinates(initial, entryDivision) {
      for (i = 0; i < values.length; i++) {
        ...
        if (initial) {
          ...
          parent.find(".graphiq__graph-dot").eq(i).attr({
            cx: xAxis,
          });
          parent
            .find(".graphiq__line")
...
angular.module("myApp", []).directive("peakChart", [
  "$window",
  function ($win) {
    var PI = Math.PI,
      sin = Math.sin,
...
angular.module("myApp2", []).directive("graph", function () {
  return {
    restrict: "A",
          ...
          angular.forEach(data, function (val, i) {
            ...
            if (psty) {
              ...
              if (typeof next === "number") {
                ...
                psty.height =
                  Math.sqrt(Math.pow(sectWidth, 2) + Math.pow(delta, 2)) + "px";
